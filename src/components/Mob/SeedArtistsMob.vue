    ...
    <div class="card2">
      <template v-for="(s,index) in d.tracks">
        <div v-if="s.preview_url && s.album.images[0]" class="con3" v-bind:key="index"
             ...
             v-on:click="store.click($event);store.deeperTracksM({item:s,num:num,flag:false,sib:'seed_artists',parent:d.id}); store.queuein(s)">
        ...
        <div v-else-if="!s.preview_url && s.album.images[0] && store.unplayable_tracks" tabindex="0" class="con3 half-opacity"
             ...
             v-on:click="store.deeperTracksM({item:s,num:num,flag:false,sib:'seed_artists',parent:d.id}); store.queuein(s)">
          {{Lists.Ls(s.artists)}} - {{ s.name }}
