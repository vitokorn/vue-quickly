...
<!--        <input type="checkbox" v-else @click.once="followTrack(d,$event)" v-model="d.followed">-->
    ...
    <template v-for="(art,index) in d.artists">
      <div class="artist-cirle con3" v-if="d.preview_url && d.album.images[0]" v-bind:key="index" v-on:click="this.$store.dispatch('deeperartistmob',{item:art,track:d,num:num,flag:false,sib:'playlisttrack',parent:item})" v-bind:style="{ 'background-image': 'url(' + d.album.images[0].url + ')' }">
        <audio preload="auto" v-bind:src="d.preview_url"></audio>
        <div style="float: left; position: absolute; font-size: 0.7em;">{{art.name}}</div>
      </div>
      <div class="artist-cirle con3" v-else-if="!d.preview_url && d.album.images[0]" v-bind:key="'2'+index" v-on:click="this.$store.dispatch('deeperartistmob',{item:art,track:d,num:num,flag:false,sib:'playlisttrack',parent:item})" v-bind:style="{ 'background-image': 'url(' + d.album.images[0].url + ')' }" style="opacity: .5">
        <audio></audio>
        <div style="float: left; position: absolute; font-size: 0.7em;">{{art.name}}</div>
      </div>
      <div class="artist-cirle con3" v-else-if="d.preview_url && !d.album.images[0]" v-bind:key="'3'+index" v-on:click="this.$store.dispatch('deeperartistmob',{item:art,track:d,num:num,flag:false,sib:'playlisttrack',parent:item})">
        <audio preload="auto" v-bind:src="d.preview_url"></audio>
        <div style="float: left; position: absolute; font-size: 0.7em;">{{art.name}}</div>
      </div>
