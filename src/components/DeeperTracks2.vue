      ...
      <div class="text-left ms-2" style="width: 50%;">
        ...
        <div class="display-flex align-items-center"><p>By </p>
          <div v-for="(art,index) in d.artists" v-bind:key="index" class="display-flex align-items-center">
            ...
            <div class="mx-1 pointer" v-on:click="store.deeperartist({item:art,track:d,num:num,flag:false,sib:'playlisttrack'})">{{art.name}}</div>
          </div>
        </div>
...
<!--          <input type="checkbox" v-else @click.once="followTrack(d,$event)" v-model="d.followed">-->
      ...
      <template v-for="(art,index) in d.artists">
        ...
        <div class="artist-cirle con3 half-opacity" v-else-if="!d.preview_url && d.images[0] && store.unplayable_tracks" v-bind:key="'2'+index" v-on:click="store.deeperartist({item:art,track:d,num:num,flag:false,sib:'playlisttrack'})" v-bind:style="{ 'background-image': 'url(' + d.images[0].url + ')' }">
        ...
        <div class="artist-cirle con3 half-opacity" v-else-if="store.unplayable_tracks" v-bind:key="'4'+index" v-on:click="store.deeperartist({item:art,track:d,num:num,flag:false,sib:'playlisttrack'})">
          <audio preload="none"></audio>
          <div class="float-left" style="position: absolute; font-size: 0.7em;">{{art.name}}</div>
        </div>
      </template>
