  ...
  <div class="seed_artists card2" v-bind:key="'sa'+index" v-bind:id="d.id">
    ...
    <div class="card2">
      <template v-for="(s,index) in d.tracks" >
        ...
        <div v-else-if="!s.preview_url && s.album.images[0]" tabindex="0" class="con3" v-bind:key="'2'+index" v-bind:style="{ 'background-image': 'url(' + s.album.images[1].url + ')' }" style="opacity: .5" v-on:click="store.deeperTracks({item:s,num:num,flag:false,sib:'seed_artists'}); store.queuein(s)">{{s.name}}
        ...
        <div v-else tabindex="0" class="con3" v-bind:key="'4'+index" style="opacity: .5" v-on:click="store.deeperTracks({item:s,num:num,flag:false,sib:'seed_artists'}); store.queuein(s)">{{s.name}}
          <audio preload="none"></audio>
        </div>
      </template>
    </div>
